...
for(col in c("HBPmeds", "diabmeds", "cholmeds", "contraceptive", "HRT")){
  medhist[[col]][medhist$medication %in% c("Prefer not to answer") | is.na(medhist$medication)] <- NA
...
# VI_diag$VIdementia <- apply(VI_diag[,grep("VeI_NonCancerCode", colnames(VI_diag), fixed=TRUE)], 1, function(x) any(x %in% dementiacodes$coding))
...
saveRDS(veint_HTNhist[,c("ID", "VIstroke", "VIhyp")], 
...
education$edu_highest <- factor(education$edu_highest, 
                                levels=c("College or University degree", "NVQ or HND or HNC or equivalent", "Other professional qualifications eg: nursing, teaching", "A levels/AS levels or equivalent", "O levels/GCSEs or equivalent", "CSEs or equivalent", "None of the above", "Prefer not to answer"),
                                ordered=TRUE)
...
# Cancel this exclusion since we're using age of leaving full-time education instead
