...
complete <- treated[!(treated$income %in% c("Do not know", "Unanswered"))
                    & treated$ISCED!="Unanswered",
                    #& !(treated$employcat %in% c("Other", "Unemployed/retired/unanswered")),
                    c("townsend_depind","townsend_quint", "income", "ISCED", "employcat", "comorbNumber_")]
for(col in colnames(complete)){
  complete[[col]] <- as.numeric(complete[[col]])
}

res <- cor(complete[,c("townsend_quint", "income", "ISCED", "comorbNumber_")], method="spearman")
kable(res)
...
tab <- table(treated$employcat, treated$income)
df <- data.frame(tab)
colnames(df) <- c("Employment_category", "Household_income", "n")

ggplot(df, aes(y=n, x=Household_income, fill=Employment_category)) +
  geom_bar(stat="identity")
