...
tab1 <- cbind(tab_c, tab_u[,3:4], tab_o[,3:4], as.matrix(uni)[,4:5])
...
# Table 2: Prevalence of hypertension control among treated hypertensives, by socioeconomic characteristics and comorbidity type

```{r table2, include=TRUE, echo=FALSE}

dflist <- c(list("Among all treated hypertensives"=treated),
            lapply(levels(treated$agegrp), function(x) treated[treated$agegrp==x,]),
            lapply(levels(treated$gender), function(x) treated[treated$gender==x,]),
            lapply(levels(treated$eth_group), function(x) treated[treated$eth_group==x,]),
            list("None"=treated[treated$comorbNone==TRUE,]), 
            lapply(comorbs_raw, function(x) treated[treated[[x]]==1,])
            )
tab2 <- c()
for(df in dflist){
  n <- nrow(df)
  ...
  pct_control <- format(round(100 * control$estimate,2), digits=1, nsmall=1)
  ci_control <- paste0("(", format(round(100 * control$conf.int[1],2), digits=1, nsmall=1), ", ", 
                       format(round(100 * control$conf.int[2],2), digits=1, nsmall=1), ")")
  tab2 <- rbind(tab2,(cbind(n, mean_age, paste0(mean_SBP,"/",mean_DBP), pct_control, ci_control)))
}

tab2 <- cbind(c("All treated hypertensives", levels(treated$agegrp), 
                levels(treated$gender), levels(treated$eth_group),
                    "None", sapply(comorbs_string, prettyfunc)),
              tab2)
rownames(tab2) <- c("", 
                    "Age groups", rep("", (nlevels(treated$agegrp)-1)), 
                    "Gender", rep("", (nlevels(treated$gender)-1)),
                    "Ethnic groups", rep("", (nlevels(treated$eth_group)-1)), 
                    "Comorbidities", rep("", length(comorbs_string)))
colnames(tab2) <- c("", "n", "Mean age", "Mean SBP/DBP", "% Controlled", "95% CI Controlled")
kable(tab2)
...
kable(tab4, caption="Logistic regressions of individual comorbidities against hypertension control status, odds ratios are for successful BP control.")
