...
<div custom-style="Note">Old and new self-reported ethnicity groupings in descriptive table, only new version in regression</div>

```{r table1, include=TRUE, echo=FALSE}
varlist=c("age", "agegrp", "gender", "ethnicity", "eth_group", "PRS",
...
dflist <- c(list("Among all hypertensives"=hypertensives),
            ...
            lapply(levels(hypertensives$eth_group), function(x) hypertensives[hypertensives$eth_group==x,]),
...
tab2 <- cbind(c("All hypertensives", levels(hypertensives$agegrp), levels(hypertensives$eth_group),
                    "None", sapply(comorbs_string, prettyfunc)),
              tab2)
rownames(tab2) <- c("", 
                    "Age groups", rep("", (nlevels(hypertensives$agegrp)-1)), 
                    "Ethnic groups", rep("", (nlevels(hypertensives$eth_group)-1)), 
...
# Table 3: Multivariable logistic regression identifying factors associated with hypertension control (BP < 140/90 mmHg), among white UK adults on antihypertensive treatment (n=`r nrow(white_trt)`)
...
n <- data.frame(descriptivetable(df=white_trt, 
                         varlist=varlist,
                         contavg='median',
                         pretty_names=pretty_names),
                stringsAsFactors = FALSE)

adj <- regressiontable(df=white_trt, outcome="controlled", varlist=varlist, 
                       regresstype="adjusted", adjvarlist=c("agegrp", "gender"))
multi <- regressiontable(df=white_trt, outcome="controlled", varlist=varlist, 
...
Among white individuals receiving treatment for hypertension  (n=`r nrow(white_trt)`), is PRS predictive of successful control?

```{r prs_regress, include=TRUE, echo=FALSE}

varlist=c("agegrp", "gender", "PRS_quint", pclist, #"eth_group",
          "townsend_quint", "income", "employcat", "ISCED", "BirthCountryIncomeLevel", "countryResidence",
          "BMIcat", "Smo_Status", "weekly_alccat", "METs_over1200",
          "antiHTNmedsno",  "FamilyHist_CVD_",
          "comorbNumber_", "VI_CVD_",
