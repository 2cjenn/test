...
toplevel <- coding87[coding87$parent_id==0,] %>%
  transmute(TL = factor(meaning),
         topcode = factor(coding),
         join_on = node_id,
         select = selectable)

level1 <- merge(toplevel, coding87, by.x="join_on", by.y="parent_id", all.x=TRUE)
...
level2 <- merge(level1, coding87, by.x="join_on", by.y="parent_id", all.x=TRUE)
...
level3 <- merge(level2, coding87, by.x="join_on", by.y="parent_id", all.x=TRUE)
...
level4 <- merge(level3, coding87, by.x="join_on", by.y="parent_id", all.x=TRUE)
