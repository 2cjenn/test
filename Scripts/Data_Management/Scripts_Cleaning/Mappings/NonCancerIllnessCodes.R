...
noncancer_illness <- function() {
  #--------------------------------------------------------------------------------------------------------------
  # Load the codings
  coding <- read.csv(file.path(config$cleaning$coding, "coding6.csv"), header=TRUE) %>%
    rename(Code = value,
           node_id = code_id)
  ...
  names(level4)[names(level4) == "medcode"] <- "Code"
  level4 <- unique(level4)
  
  # Coalesce to get lowest level diagnosis per code
  level4$dx <- coalesce(level4$L4, level4$L3, level4$L2, level4$L1, level4$TL)
  ...
  write.csv(level4, file.path(config$data$derived, "NonCancerIllness_Map.csv"), 
            na="", row.names=FALSE)
