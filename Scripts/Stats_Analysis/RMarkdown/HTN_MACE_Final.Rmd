...
MI_data=data%>%
  ...
  mutate(TEU_SBP_PRS=1740*TEU_SBP_PRS) %>% 
  droplevels()
...
if(live) {
  ...
  imp1 <- readRDS(file.path(config$data$derived, "imputation18May2021_newPRS.rds"))
}
```
...
tab_mace <- cox_a_b(varlist_a, varlist_b, status="TEU_MACE_status", displaycols="TEU_SBP_PRS_quintiles")$tab
tab_mi <- cox_a_b(varlist_a, varlist_b, status="TEU_MACE_MI", displaycols="TEU_SBP_PRS_quintiles")$tab
tab_stroke <- cox_a_b(varlist_a, varlist_b, status="TEU_MACE_Stroke", displaycols="TEU_SBP_PRS_quintiles")$tab

tab <- rbind(c("Outcome", "Variable", "HR", "95% CI", "p", "HR", "95% CI", "p"),
             cbind(c(paste0("MACE, n=", nrow(data[data$TEU_MACE_status==1,])), rep("", nrow(tab_mace)-2)),tab_mace[-1,]), 
             cbind(c(paste0("MI, n=", nrow(data[data$TEU_MACE_MI==1,])), rep("", nrow(tab_mi)-2)),tab_mi[-1,]), 
             cbind(c(paste0("Stroke, n=", nrow(data[data$TEU_MACE_Stroke==1,])), rep("", nrow(tab_stroke)-2)),tab_stroke[-1,]))
