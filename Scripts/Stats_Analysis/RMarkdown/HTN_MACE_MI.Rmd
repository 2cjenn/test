...
if(live) {
  ...
  imp1<-mice(MI_data,pred=pred, m=10, maxit = 20,seed = 100)
  
  save(imp1,file=file.path(config$data$derived, paste0("imputation", format(Sys.time(), '%d%B%Y'), ".rds")))
} else {
  imp1 <- readRDS(file.path(config$data$derived, "imputation10March2021.rds"))
}
```
...
for (var in missing_var){
  ...
  if (is.numeric(MI_data[[var]])){
    ...
    long1 <- long1 %>%
      mutate(ind.na=rep(is.na(imp1$data[[var]]),5))
    ...
    long1 <- long1 %>%
      mutate(ind.na=rep(ifelse(is.na(imp1$data[[var]]),'Imputed','Observed'),5))
...
lapply(1:5, function(i) summary(long1%>%filter(.imp==i)%>%pull(TEU_BlP_SBP.avg)))

summary(MI_data$TownsendDepInd)

lapply(1:5, function(i) summary(long1%>%filter(.imp==i)%>%pull(TownsendDepInd)))
...
anova <- as.formula(paste0("TEU_controlledHTN ~ ",
                             paste(varlist[-which(varlist=="TEU_SBP_PRS_quintiles")], collapse="+")))
SBP_p <- anova(multi, glm(anova, data=data, family="binomial"), test="Chisq")$`Pr(>Chi)`[2]
...
# Model A
formula_a <- as.formula(paste0("Surv(TEU_MACE_time_yrs,", status, ") ~ ", paste(varlist_a, collapse="+")))
...
# Model B
formula_b <- as.formula(paste0("Surv(TEU_MACE_time_yrs,", status, ") ~ ", paste(varlist_b, collapse="+")))
...
anova_a <- as.formula(paste0("Surv(TEU_MACE_time_yrs,", status, ") ~ ",
                             paste(varlist_a[-which(varlist_a=="TEU_SBP_PRS_quintiles")], collapse="+")))
anova_b <- as.formula(paste0("Surv(TEU_MACE_time_yrs,", status, ") ~ ",
                             paste(varlist_b[-which(varlist_b=="TEU_SBP_PRS_quintiles")], collapse="+")))

SBP_p_a <- anova(rmodel_a, coxph(anova_a, data=data), test="Chisq")$`P(>|Chi|)`[2]
SBP_p_b <- anova(rmodel_b, coxph(anova_b, data=data), test="Chisq")$`P(>|Chi|)`[2]
