...
tab <- inner_join(tab_1, tab_3%>%select(-Variable), by="IDcol")
tab <- inner_join(tab, tab_5%>%select(-Variable), by="IDcol")
tab <- inner_join(tab, tab_all%>%select(-Variable), by="IDcol")

rownames(tab) <- NULL
tab <- rbind(c("", "Variable", "n", "%", "n", "%", "n", "%", "n", "%"),
              as.matrix(tab))
colnames(tab) <- c("", "", "Q1: lowest", "", "Q3", "", "Q5: highest", "", "Overall", "")
...
routput_a$HR[grepl("TEU_SBP_PRS_quintiles", routput_a$IDcol, fixed=TRUE)] <- NA
