...
varlist1=c("TEU_BaC_AgeCat", "BaC_Sex", "TEU_SBP_PRS_quintiles", #"TEU_BlP_SBP_quintiles", 
          "TEU_BSM_BMIcat", "TEU_Smo_Status", "TEU_Alc_WeeklyCat", "TEU_Pha_METsover1200",
          "TEU_FaH_CVD", "Prosp_comorb_numcat", "TEU_VeI_numHTNmedscat", "TEU_LDL_Quintiles",
          "TEU_TownsendDepInd_Quint", "TEU_HouseholdIncome", "TEU_Emp_category", "TEU_Edu_ISCED", "TEU_Rec_Country",
          "GeP_PC_1", "GeP_PC_2", "GeP_PC_3", "GeP_PC_4"
...
varlist2=c("TEU_BaC_AgeCat", "BaC_Sex", "TEU_BlP_SBP_quintiles", "TEU_SBP_PRS_quintiles", 
          "TEU_BSM_BMIcat", "TEU_Smo_Status", "TEU_Alc_WeeklyCat", "TEU_Pha_METsover1200",
          "TEU_FaH_CVD", "Prosp_comorb_numcat", "TEU_VeI_numHTNmedscat", "TEU_LDL_Quintiles",
          "TEU_TownsendDepInd_Quint", "TEU_HouseholdIncome", "TEU_Emp_category", "TEU_Edu_ISCED", "TEU_Rec_Country",
          "GeP_PC_1", "GeP_PC_2", "GeP_PC_3", "GeP_PC_4"
...
# chisq.test(table(ordered(data$TEU_SBP_PRS_quintiles), data$TEU_MACE_status))
# 
# prop.test(table(ordered(data$TEU_SBP_PRS_quintiles), data$TEU_MACE_status))
# 
# prop.trend.test(table(data$TEU_SBP_PRS_quintiles[data$TEU_MACE_status==1]),
#                 table(data$TEU_SBP_PRS_quintiles))

```

```{r, eval=live}

# minimodel <- coxph(Surv(TEU_MACE_time_yrs,TEU_MACE_status) ~ TEU_BaC_AgeCat + BaC_Sex + TEU_SBP_PRS_quintiles, data=data)
# thing <- publish(minimodel, ci.digits=2, ci.handler="sprintf", pvalue.eps=0.001,
#               pvalue.digits=2, pvalue.stars=FALSE, ci.trim=FALSE, print=FALSE)$rawTable
# 
#  publish(minimodel, ci.digits=2, ci.handler="sprintf", pvalue.eps=0.001,
#               pvalue.digits=2, pvalue.stars=FALSE, ci.trim=FALSE, print=FALSE)$regressionTable
#  thing
# 
# format.pval(0.00039485, digits=3, eps=0.001)

test.ph <- cox.zph(rmodel2)
ggcoxzph(test.ph)
...
```{r Model 1 & 2 Model Checking,eval=FALSE,echo=FALSE}
...
shinyApp(ui = fluidPage(
  mainPanel(
    plotOutput("main_plot",height = "1000px",width = "2000px")
  )),
  server = function(input, output) {
    
    output$main_plot <- renderPlot({
      ftest<-cox.zph(rmodel1)
      ggcoxzph(ftest)}, height = 1000, width = 2000 )
    
  })

## 2. Multicollinearity
# VIF
vif=rms::vif(cph(Surv(TEU_MACE_time_yrs,TEU_MACE_status)~ TEU_BaC_AgeCat + BaC_Sex +  TEU_LDL_C_PRS_Quint  + 
               TEU_BSM_BMIcat + TEU_Smo_Status+
               TEU_Alc_WeeklyCat + TEU_Pha_METsover1200 + TEU_FaH_CVD + 
               Prosp_comorb_numcat + TEU_BlP_SBP_Quint+
               TEU_TownsendDepInd_Quint + TEU_HouseholdIncome + TEU_Emp_category + TEU_Edu_ISCED + TEU_Rec_Country+
               GeP_PC_1 + GeP_PC_2 + GeP_PC_3 + GeP_PC_4,data=model1_data))
...
shinyApp(ui = fluidPage(
  ...
  server = function(input, output) {
    ...
    output$main_plot <- renderPlot({
      ftest2<-cox.zph(rmodel2)
      ggcoxzph(ftest2)}, height = 1000, width = 2000 )
...
vif=rms::vif(cph(Surv(TEU_MACE_time_yrs,TEU_MACE_status)~ TEU_BaC_AgeCat + BaC_Sex +  TEU_LDL_C_PRS_Quint  + 
               TEU_BSM_BMIcat + TEU_Smo_Status+
               TEU_Alc_WeeklyCat + TEU_Pha_METsover1200 + TEU_FaH_CVD + 
               Prosp_comorb_numcat + TEU_BlP_SBP_Quint+  BBC_LDL_Quint+
               TEU_TownsendDepInd_Quint + TEU_HouseholdIncome + TEU_Emp_category + TEU_Edu_ISCED + TEU_Rec_Country+
               GeP_PC_1 + GeP_PC_2 + GeP_PC_3 + GeP_PC_4,data=model2_data))


# 3. Check correlation among covariates 
corr <- data%>%select(varlist2)%>%select(!starts_with("GeP_PC"))
...
## 1. PHA
ftest<-cox.zph(rmodel1)
# Using shinyApp for better plot visualisation
shinyApp(ui = fluidPage(
  mainPanel(
    plotOutput("main_plot",height = "1000px",width = "2000px")
  )),
...
cor(as.numeric(data$TEU_TownsendDepInd_Quint[data$TEU_TownsendDepInd_Quint!="Unavailable" & !data$TEU_HouseholdIncome%in%c("Prefer not to answer", "Unanswered")]), as.numeric(data$TEU_HouseholdIncome[data$TEU_TownsendDepInd_Quint!="Unavailable" & !data$TEU_HouseholdIncome%in%c("Prefer not to answer", "Unanswered")]), method="spearman")
