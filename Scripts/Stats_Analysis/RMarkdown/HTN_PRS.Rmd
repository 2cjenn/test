...
varlist=c("TEU_BaC_AgeAtRec", "TEU_BaC_AgeCat", "BaC_Sex", "TEU_BlP_SBP_quintiles",
          "TEU_HouseholdIncome", "TEU_Emp_category", "TEU_Edu_ISCED", "TEU_Rec_Country", "TEU_TownsendDepInd_Quint",
          "TEU_BSM_BMIcat", "TEU_Smo_Status",
          "TEU_VeI_numHTNmedscat", "TEU_VeI_CVD", "TEU_VeI_diab",
          "TEU_FaH_CVD", "TEU_LDL_Quintiles",
          "Prosp_comorb_numcat",
...
## Figure `r figure`: Forest plot of association between BP quintiles and hypertension control in age- and sex-adjusted and multivariable logistic regression models, among middle-aged UK adults on anti-hypertensive treatment (n=`r nrow(data)`)
`r figure <- figure + 1`

```{r forest plot, include=TRUE, echo=FALSE}
...
summ <- summary(agesex)
conf <- summ$coeff
conf[,3] <- exp(conf[,1] - (1.96*conf[,2]))
conf[,4] <- exp(conf[,1] + (1.96*conf[,2]))
conf[,1] <- exp(conf[,1])
unctrl <-
  data.frame(
    hr = c(1, conf[rownames(conf) %in% c(
      "TEU_BP_PRS_quintilesQ2",
      "TEU_BP_PRS_quintilesQ3",
      "TEU_BP_PRS_quintilesQ4",
      "TEU_BP_PRS_quintilesQ5: highest"
    ), 1]),
    lci = c(1, conf[rownames(conf) %in% c(
      "TEU_BP_PRS_quintilesQ2",
      "TEU_BP_PRS_quintilesQ3",
      "TEU_BP_PRS_quintilesQ4",
      "TEU_BP_PRS_quintilesQ5: highest"
    ), 3]),
    uci = c(1, conf[rownames(conf) %in% c(
      "TEU_BP_PRS_quintilesQ2",
      "TEU_BP_PRS_quintilesQ3",
      "TEU_BP_PRS_quintilesQ4",
      "TEU_BP_PRS_quintilesQ5: highest"
...
# tabletext <- 
tabletext <- cbind(c("Model", "Age- and sex-adjusted", "Q1 (lowest)", "Q2", "Q3", "Q4", "Q5 (highest)",
               "Multivariable", "Q1 (lowest)", "Q2", "Q3", "Q4", "Q5 (highest)"),
               c("OR", NA, round(unctrl$hr,2), NA, round(ctrl$hr,2))
)

forestplot(tabletext,
           hrzl_lines = list("1" = gpar(lty = 1), 
                             "2" = gpar(lty = 1),
                             "3" = gpar(lty = 1, columns = 1), 
                             "8" = gpar(lty = 1),
                             "9" = gpar(lty = 1, columns = 1)
                             ),
           mean = c(NA, NA, unctrl$hr, NA, ctrl$hr),
           lower = c(NA, NA, unctrl$lci, NA, ctrl$lci),
           upper = c(NA, NA, unctrl$uci, NA, ctrl$uci),
           col = fpColors(box = c("navy")),
           xlab = "OR for hypertension control",
           vertices=TRUE,
           is.summary = c(TRUE,TRUE,rep(FALSE,5),TRUE,rep(FALSE,5)),
           zero=1,
           graph.pos=2,
           xticks = c(0.6, 0.7, 0.8, 0.9, 1.0, 1.1),
           boxsize=0.6,
           title="OR for HTN control by quintile of BP PRS",
           txt_gp = fpTxtGp(label = gpar(fontsize = 10))
           )

```

## Figure `r figure`: ROC curve - control status under multivariable model
`r figure <- figure + 1`

```{r}
#https://daviddalpiaz.github.io/r4sl/logistic-regression.html

data_roc <- data %>% mutate(TEU_controlledHTN = factor(as.numeric(TEU_controlledHTN))) #%>%
  #filter(!is.na(TEU_TownsendDepInd_Quint))
formula <- paste0("TEU_controlledHTN ~ ", paste(varlist, collapse=" + "))
model <- glm(formula, data=data_roc, family="binomial")

data_roc$test_prob = predict(model, newdata = data_roc, type = "response")
test_roc = pROC::roc(data_roc$TEU_controlledHTN ~ data_roc$test_prob, plot = TRUE, print.auc = TRUE, 
               levels=c(0,1), direction="<")

# library(cutpointr)
# cp <- cutpointr(data_roc, test_prob, TEU_controlledHTN, 
...
freq <- table(data$TEU_MACE_fucomp)

# Proportion table
prop <- data.frame(propped(freq))

# Add person years in data to frequencies
freq <- cbind(freq, rep(sum(data$TEU_MACE_time_yrs), nrow(freq)))

# Incidence per thousand person years
inc <- epi.conf(freq, ctype = "inc.rate", method = "exact", N = nrow(data), design = 1, 
   conf.level = 0.95) * 1000

inc <- data.frame(round(inc, 2))
inc$Var1 <- rownames(inc)
inc$Inc <- paste0(round(inc$est, 2), " (95% CI: ", round(inc$lower, 2), " to ", round(inc$upper, 2), ")")


# Combined table
MACE_subtypes <- inner_join(prop, inc, by="Var1") %>% select(Var1, Freq, Inc)
colnames(MACE_subtypes) <- c('MACE subtypes', "Frequency", "Incidence per thousand person-years")

freq <- data.frame(freq) %>% mutate(Freq = freq, Var1 = rownames(.))
kable(MACE_subtypes,caption=paste0('Table.1 Frequency of MACE subtypes (N=',sum(freq$Freq),')'))

  
```

During follow-up (median `r median(data$TEU_MACE_time_yrs)` years), there were `r sum(freq$Freq)` incident MACE events (incidence rate `r sum(inc$est)` per 1000 person-years), of which `r freq$Freq[freq$Var1=="Nonfatal MI"]` were non-fatal MI (incidence rate `r inc$est[inc$Var1=="Nonfatal MI"]` per 1000 person-years), `r sum(freq$Freq[grep("Nonfatal Stroke", freq$Var1)])` were non-fatal stroke (incidence rate `r sum(inc$est[grep("Nonfatal Stroke", inc$Var1)])` per 1000 person-years) and `r freq$Freq[freq$Var1=="CVD death"]` were CVD-related death (incidence rate `r inc$est[inc$Var1=="CVD death"]` per 1000 person-years). 
...
varlist=c("TEU_BaC_AgeCat", "BaC_Sex", "TEU_BP_PRS_quintiles", "TEU_BlP_SBP_quintiles",
          "TEU_TownsendDepInd_Quint", "TEU_HouseholdIncome", "TEU_Emp_category", "TEU_Edu_ISCED", "TEU_Rec_Country",
          "TEU_BSM_BMIcat", "TEU_Smo_Status", "TEU_Alc_WeeklyCat", "TEU_Pha_METsover1200",
          "TEU_VeI_numHTNmedscat",  "TEU_FaH_CVD", "TEU_LDL_Quintiles",
          "Prosp_comorb_numcat",
          "GeP_PC_1", "GeP_PC_2", "GeP_PC_3", "GeP_PC_4")
...
## Cox regression of the time until haemorrhagic stroke event (`r nrow(data[data$TEU_MACE_HaemStroke==1,])` events)
...
# age- and sex-adjusted
agesex <- coxph(Surv(TEU_MACE_time_yrs,TEU_MACE_HaemStroke) ~ TEU_BaC_AgeCat + BaC_Sex, data=data)
adj <- publish(agesex, ci.digits=2, ci.handler="sprintf", pvalue.eps=0.01,
              pvalue.digits=2,pvalue.stars=TRUE, ci.trim=FALSE,print=FALSE)$regressionTable
for(var in varlist){
  if(! var %in% c("TEU_BaC_AgeCat", "BaC_Sex")){
  formula <- as.formula(paste0("Surv(TEU_MACE_time_yrs,TEU_MACE_HaemStroke) ~ TEU_BaC_AgeCat + BaC_Sex + ", var))
  model <- coxph(formula, data=data)
  output <- publish(model, ci.digits=2, ci.handler="sprintf", pvalue.eps=0.01,
              pvalue.digits=2,pvalue.stars=TRUE, ci.trim=FALSE,print=FALSE)$regressionTable
  output <- output[-c(1:5),] %>% select(-any_of("Missing"))
...
# Complete case data
formula <- as.formula(paste0("Surv(TEU_MACE_time_yrs,TEU_MACE_HaemStroke) ~ ", paste(varlist, collapse="+")))
