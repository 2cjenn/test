...
forestplot(tabletext,
           ...
           boxsize=0.6,
           title="Odds ratios for hypertension control by quintile of BP PRS"
           )
...
data_roc <- data %>% mutate(TEU_controlledHTN = as.numeric(TEU_controlledHTN))
formula <- paste0("TEU_controlledHTN ~ ", paste(varlist, collapse=" + "))
model <- glm(formula, data=data_roc, family="binomial")

test_prob = predict(model, newdata = data_roc, type = "response")
test_roc = roc(data_roc$TEU_controlledHTN ~ test_prob, plot = TRUE, print.auc = TRUE, 
               levels=c(0,1), direction="<", main="ROC curve: control status under multivariable model")
