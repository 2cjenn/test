...
# The change in root directory will apply in all subsequent chunks, but not in this chunk
# So to load files and source code in this chunk, we need to specify the directory using here()
config <- yaml.load_file(here::here("config.yml"))
source(here::here(config$functions))
source(here::here(file.path(config$scripts$processing, "Neo_exclusions.R")))
rawdata <- readRDS(here::here(file.path(config$data$derived, "HTN_raw.rds")))

data <- in_dir(here::here(), Neo_HTN_all(rawdata))
treated <- in_dir(here::here(), Neo_HTN_trt(data))

# treated <- readRDS(file=here::here(file.path(config$data$analysed, "HTN_trt.rds")))
# data <- readRDS(file=here::here(file.path(config$data$analysed, "HTN_excl.rds")))
...
varlist=c("age", "agegrp", "gender", "eth_group",
          ...
          "BMIcat", "Smo_Status",
          "HTNdx_duration", "antiHTNmedsno", "VI_CVD_", "VI_diabetes_",
          "comorbNumber_")
...
# Supplementary Figure S`r suppfig`. Scatterplot of blood pressure among treated hypertensives (n = `r nrow(treated)`), categorised according to BP threshold 140/90 mmHg.
`r suppfig <- suppfig + 1`
...
# Supplementary Figure S`r suppfig`. Prevalence of hypertension categories by gender and age among UKB participants with complete BP data and no serious comorbidities (n = `r nrow(data)`).
`r suppfig <- suppfig + 1`
...
# Supplementary Figure S`r suppfig`. Bland-Altman plots of distribution of first and second blood pressure measurements among treated hypertensives (n = `r nrow(treated)`)
`r suppfig <- suppfig + 1`
...
ggplot(data=treated, aes(x=SBP, y=SBPdiff)) + geom_point() +
...
ggplot(data=treated, aes(x=DBP, y=DBPdiff)) + geom_point() +
  geom_hline(yintercept=0, color="red") +
