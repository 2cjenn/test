...
pct_ctrl <- c()
for(var in varlist){
  prettyvar <- prettyfunc(var, pnames=pretty_names, upper=TRUE)
  for(level in levels(data[[var]])){
    df <- data[data[var]==level,]
    medianfu <- pretty_dp(median(df[["stime"]]),2)
    events <- sum(df[["status"]])
    n <- nrow(df)
    newrow <- c(prettyvar, level, n, medianfu, events)
    prettyvar <- ""
    pct_ctrl <- rbind(pct_ctrl, newrow)
  }
}
colnames(pct_ctrl) <- c("Variable", "Levels", "n", "Median follow-up time, years", "Number of CVD events")
