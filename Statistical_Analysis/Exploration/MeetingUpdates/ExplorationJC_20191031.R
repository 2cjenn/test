...
# The APOE SNP data is located in K:\TEU\UKB_Genetic_Data\SNP Extraction\APOE_20191018
...
data <- readRDS("K:/TEU/UKB_Genetic_Data/SNP_Extraction_2019/APOE_20191018/extracted_files/R/gen_apoe.rds")

# Check prevalence of APOE e4 allele - expect ~20% population carriers, ~1.8% homozygotes
for (i in 1:4){
  apoe_col = paste0("apoe", i)
  tab_e <- table(data[[apoe_col]], useNA="ifany")
  print(tab_e)
  print(100*prop.table(table(data[[apoe_col]])))
}
prop.table(table(data$apoe_gen))
...
#--------------------------------------------------------------------------------------------------------------
# Dementia diagnoses
load("K:/TEU/CancerPRS/Data_Downloads/v1.0_B2004077_R37981/SelectedConvR_114fields/dementia.Rdata")
