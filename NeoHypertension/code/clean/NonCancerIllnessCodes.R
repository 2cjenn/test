...
joinables <- coding6[,c("coding", "meaning", "node_id", "parent_id")]
...
toplevel <- coding6[,c("coding", "meaning", "node_id")][coding6$parent_id==0,]
...
level4$medcode <- coalesce(level4$coding, level4$medcode)
level4 <- level4[,c("medcode", "TL", "L1", "L2", "L3", "meaning")]
names(level4)[names(level4) == "meaning"] <- "L4"
names(level4)[names(level4) == "medcode"] <- "coding"
...
write.csv(level4, "K:\\TEU\\APOE on Dementia\\Data Management\\R_Dataframes_TLA\\38358\\Organised\\NonCancerIllness.csv", 
          na="", row.names=FALSE)

# And save it as an R data file, why not
saveRDS(level4, "K:\\TEU\\APOE on Dementia\\Data Management\\R_Dataframes_TLA\\38358\\Organised\\NonCancerIllness.rds")


#--------------------------------------------------------------------------------------------------------------
# Finally, join the coded participant data to the code labels
# Load the long-format verbal interview data
veint <- readRDS("K:\\TEU\\APOE on Dementia\\Data Management\\R_Dataframes_TLA\\38358\\Organised\\VIDiagnosisCodes_long.rds")
# And merge
...
saveRDS(final, "K:\\TEU\\APOE on Dementia\\Data Management\\R_Dataframes_TLA\\38358\\Organised\\NonCancerIllness_pts.rds")
